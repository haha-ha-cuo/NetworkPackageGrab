...
PacketType PacketFactory::identifyPacketType(const std::vector<uint8_t> &data)
{
    if (data.size() < 14)
...
std::unique_ptr<Packet> PacketFactory::createPacket(const std::vector<uint8_t> &data)
{
    PacketType type = identifyPacketType(data);
    switch (type)
    {
    case PacketType::ETHERNET:
        return std::make_unique<Ethernet>(data);
    case PacketType::IP:
    {
        if (data.size() > 14)
        {
            std::vector<uint8_t> ip(data.begin() + 14, data.end());
            return std::make_unique<IP>(ip);
        }
        return nullptr;
    }
    case PacketType::TCP:
    {
        if (data.size() > 34)
        {
            ...
            {
                std::vector<uint8_t> tcp(data.begin() + tcpOffset, data.end());
                return std::make_unique<TCP>(tcp);
            }
        }
        return nullptr;
    }
    case PacketType::UDP:
    {
        if (data.size() > 34)
        {
            ...
            {
                std::vector<uint8_t> udp(data.begin() + udpOffset, data.end());
                return std::make_unique<UDP>(udp);
            }
        }
        return nullptr;
    }
